      ...
      const TimeStep = ({ item }) => {
        return (
          <div className="flex flex-col">
            <p className={` text-sm self-start my-3 ${item.id<activated?"text-[#EB1C74]":"text-slate-400"} max-xl:hidden`}>
              {item.title}
            </p>
            <div className="flex flex-row items-center">
              <Dot item={item} />
              {item.title !="Poliçe" && (
                <div className={`xl:w-20 w-10 h-[2px] ${item.id<activated-1 ? "bg-[#EB1C74]" : "bg-slate-400"}`} />
              )}
            </div>
          </div>
        );
